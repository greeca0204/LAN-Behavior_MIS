...
if($searchEnable == '') {
	$rs = mysql_query("select count(*) from tactics_user_group where Name like '%$searchName%' and Gid like '%$searchGid%' and Tactics_type in (select Tactics_type from tactics where Comment like '%$searchTactics%') and (Detail like '%$searchDetail%' or Detail in (select Name from app_table where Comment like '%$searchDetail%'))");
	$row = mysql_fetch_row($rs);
	$result["total"] = $row[0];
	$rs = mysql_query("select * from tactics_user_group where Name like '%$searchName%' and Gid like '%$searchGid%' and Tactics_type in (select Tactics_type from tactics where Comment like '%$searchTactics%') and (Detail like '%$searchDetail%' or Detail in (select Name from app_table where Comment like '%$searchDetail%')) order by $sort $order limit $offset,$rows");
}
else {
	$rs = mysql_query("select count(*) from tactics_user_group where Name like '%$searchName%' and Gid like '%$searchGid%' and Tactics_type in (select Tactics_type from tactics where Comment like '%$searchTactics%') and (Detail like '%$searchDetail%' or Detail in (select Name from app_table where Comment like '%$searchDetail%')) and Enable like '$searchEnable'");
	$row = mysql_fetch_row($rs);
	$result["total"] = $row[0];
