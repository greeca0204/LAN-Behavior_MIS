...
//进行IP分组，相同的记录不输出
$rs = mysql_query("select count(distinct IP) from attachment where UorD=1 and Uid like '%$searchUid%' and Name like '%$searchName%' and IP like '%$searchIP%'");
$array = mysql_fetch_array($rs);
$result["total"] = $array[0];
$rs = mysql_query("select * from attachment where UorD=1 and Uid like '%$searchUid%' and Name like '%$searchName%' and IP like '%$searchIP%' group by IP order by $sort $order limit $offset, $rows");
