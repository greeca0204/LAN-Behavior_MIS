...
<HEAD>
	<TITLE>
	FusionCharts v3 Demo Application - Yearly Sales By Categories
	</TITLE>
	<?php
	...
	*/
	 ...
	 * and then send request for new XML data to update the bottom chart. 
	 *	@param	year		Year for which we need the dta
	 *	@param	categoryId	Category for which we need drill down
	*/		
	function updateChart(year, catId){			
		//Update the chart if it has loaded
		if (detailsChartLoaded){
			//DataURL for the chart
			var strURL = "Data_SalesByCatProd.php?year=" + year + "&catId=" + catId;			
			...
			//Get reference to chart object using Dom ID "ProductDetails"
			var chartObj = getChartFromId("ProductDetails");			
...
<table width='875' align='center' cellspacing='0' cellpadding='0'>
	...
	<td align='center'>
	<?php
	$strXML  = "<chart caption='Sales By Category for " . $intYear . "' palette='" . getPalette() . "' animation='" . getAnimationState() . "' formatNumberScale='0' PYAxisName='Revenue' SYAxisName='Units Sold' numberPrefix='$' labelDisplay='STAGGER' seriesNameInToolTip='0' showValues='0' plotSpacePercent='10'>";
...
<P align='center' class='text'>Select a category above to view product wise sales.</P>
...
<td align='center'>
	...
	echo renderChart("FusionCharts/MSColumn3DLineDY.swf?ChartNoDataText=Please select a category above.", "", "<chart></chart>", "ProductDetails", 850, 300, false, true);
	?>
	</td>
	</tr>
</table>	
